...
<main className={`${config.font} ${config.theme.pageBg} ${config.theme.pageText}`}>
  ...
  <div className={`mx-auto ${siteConfig.maxWidth} ${siteConfig.pagePadding}`}>
    <section className={`${config.theme.sectionBg} p-8 mb-12`}>
      ...
      <div className="grid md:grid-cols-2 gap-6"><div className={`${config.theme.cardBg} p-6`}><h3 className="text-2xl font-semibold mb-3">Our Story</h3><p className="text-gray-700">{config.companyStory}</p></div><div className={`${config.theme.cardBg} p-6`}><h3 className="text-2xl font-semibold mb-3">Our Mission</h3><p className="text-gray-700">{config.mission}</p></div><div className={`${config.theme.cardBg} p-6`}><h3 className="text-2xl font-semibold mb-3">What We Do</h3><p className="text-gray-700">{config.whatWeDo}</p></div><div className={`${config.theme.cardBg} p-6`}><h3 className="text-2xl font-semibold mb-3">Areas We Serve</h3><p className="text-gray-700">{config.serviceAreas.join(", ")}</p></div><div className={`${config.theme.cardBg} p-6 md:col-span-2`}><h3 className="text-2xl font-semibold mb-3">Why Choose Us</h3><p className="text-gray-700">{config.whyChooseUs}</p></div><div className={`${config.theme.cardBg} p-6 md:col-span-2`}><h3 className="text-2xl font-semibold mb-3">Socials</h3><p className="text-gray-700 mb-4">Follow Tabor Painting on these platforms once our social pages are launched.</p><div className="flex flex-wrap gap-3">{socials.map((social) => (<span key={social} className="rounded-full border border-yellow-600/30 bg-yellow-50 px-4 py-2 text-sm font-semibold text-yellow-800">{social}</span>))}</div></div></div>
