    ...
    <main className={`${config.font} ${config.theme.pageBg} ${config.theme.pageText}`}>
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-12">
        <div className="text-center mb-10 md:mb-20">
          ...
          <p className="text-lg md:text-xl max-w-4xl mx-auto text-gray-700 leading-relaxed">{servicesConfig.pageSubtitle}</p>
        </div>
          ...
          {servicesConfig.services.map((service, index) => (
            <section key={service.title} className={`${config.theme.sectionBg} grid md:grid-cols-2 gap-6 md:gap-10 items-center p-5 md:p-12`}>
              <div className={index % 2 === 1 ? "md:order-2" : ""}>
                ...
                <p className="text-base md:text-lg leading-relaxed text-gray-700">{service.description}</p>
              </div>
      ...
      <footer className="border-t border-yellow-600/30 py-8 text-center px-4">
        <p>{config.email}</p>
        ...
        <Link href={siteConfig.privacyPageRoute} className="inline-block mt-4 text-yellow-700 hover:text-black">Privacy Policy</Link>
      </footer>
    </main>
  );
}
